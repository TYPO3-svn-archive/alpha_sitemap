...
class tx_alphasitemap_pi1 extends tslib_pibase {
	...
	public function main($content, $conf)	{
		...
		//0-9
		$charlinks .= $this->cObj->stdWrap($aChar[' '] ? $this->pi_linkTP('0-9', array($this->prefixId.'[char]' => 0), 1) : '0-9', $this->conf['chars.']);
		//chars
		for ($i = 65; $i < 91; $i++) {
		...
		if ($this->piVars['char']) {
			...
			foreach($items as $ch => $val) {
				if ($val) {
					$totalMenu .= $this->cObj->stdWrap($ch == ' ' ? '0-9' : $ch, $this->conf['titleChar.']);
	...
	* @return
	*/
	protected function recursiveMenu($pid, $level = 1) {
